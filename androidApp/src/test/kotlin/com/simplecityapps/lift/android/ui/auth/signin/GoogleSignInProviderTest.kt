...
class GoogleSignInProviderTest {
    ...
    fun setup() {
        ...
        googleSignInProvider = GoogleSignInProvider(authManager, googleSignInClient)
    ...
    fun `test handleSignInResult success`() = runBlocking {
        ...
        // Assert
        coVerify { authManager.signIn(idToken) }
    }
    ...
    fun `test handleSignInResult failure due to null idToken`() = runBlocking {
        ...
        // Assert
        verify { authManager.updateAuthState(match { it is AsyncState.Failure }) }
    }
    ...
    fun `test handleSignInResult failure due to ApiException`() = runBlocking {
        ...
        // Assert
        verify { authManager.updateAuthState(AsyncState.Failure(exception)) }
    }
}