...
class ExerciseScreenPreviewProvider :
    PreviewParameterProvider<Pair<Colors, ExerciseScreenViewModel>> {
    override val values: Sequence<Pair<Colors, ExerciseScreenViewModel>>
        get() = ThemedPreviewProvider().values.map { colors ->
            colors to ExerciseScreenViewModel(DummyAppRepository()).apply {
                this.exerciseProgressMap[DummyAppRepository.routineExercises.first()] =
                    ExerciseScreenViewModel.ExerciseProgress.InProgress(2)
